...
**Important — keep the world-space contact correction centralized.** Task 2's `Circle.sweep` added `toi.point += targetDelta * toi.t` to convert the rest-frame contact point back to world space. That correction now moves into the shared `sweepConvex` (which takes `movedBy:`), so all four shapes inherit it and it is NOT duplicated in the public methods. The Task-2 `Circle.sweep(against: Circle, movedBy:)` body is fully replaced by the `against: CollisionShape` version below.
...
Note: deleting the Task-2 `Circle.sweep(against: Circle, movedBy:)` overload is required — the new `against: CollisionShape` method subsumes it (a `Circle` argument still binds, since `Circle: CollisionShape`). The Task-2 tests (including `sweepBothMovingReportsWorldSpaceContact`, which checks the world-space contact point) keep passing unchanged because the correction is preserved in `sweepConvex`.
...
Then add the `Halfspace` case to the `sweepConvex` switch (insert before `default`). Note the switch assigns to `result` (the shared world-space contact correction runs afterward), so use assignment, not `return`:
