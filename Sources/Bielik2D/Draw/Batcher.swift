...
final class Batcher {
    ...
    public func emitQuad(rect: Rect, uv: Rect, color: SIMD4<Float>,
                         scaleData: SIMD4<Float> = .zero, uvBounds: SIMD4<Float> = .zero) {
        emitQuadCorners(
    ...
    public func emitQuadCorners(p0: SIMD2<Float>, p1: SIMD2<Float>, p2: SIMD2<Float>, p3: SIMD2<Float>,
        ...
        vertices.append(Vertex(pos: p0, uv: u0, color: color, attributes: scaleData, uvBounds: uvBounds))
        vertices.append(Vertex(pos: p1, uv: u1, color: color, attributes: scaleData, uvBounds: uvBounds))
        vertices.append(Vertex(pos: p2, uv: u2, color: color, attributes: scaleData, uvBounds: uvBounds))
        vertices.append(Vertex(pos: p0, uv: u0, color: color, attributes: scaleData, uvBounds: uvBounds))
        vertices.append(Vertex(pos: p2, uv: u2, color: color, attributes: scaleData, uvBounds: uvBounds))
        vertices.append(Vertex(pos: p3, uv: u3, color: color, attributes: scaleData, uvBounds: uvBounds))
    }
